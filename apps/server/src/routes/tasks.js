 ...
 *                 type: string
 *                 example: "HBAR"
 *                 description: Use `HBAR` for native transfers or an HTS symbol such as `USDC-HTS` or `CASH-HTS`.
 *               tokenId:
 *                 type: string
 *                 example: "0.0.456789"
 *                 description: Required when `currency` is not HBAR.
 *               tokenDecimals:
 *                 type: integer
 *                 example: 6
 *     responses:
...
router.post("/:id/pay", requireAuth, async (req, res, next) => {
  try {
    ...
    try {
      const currency =
        optionalString(req.body?.currency, "currency", { max: 32 }) || "HBAR";
      const tokenId = optionalString(req.body?.tokenId, "tokenId", {
        max: 64,
      });
      ...
      return res.json({
        ...
        currency: result.payment.currency,
        tokenId: result.payment.token_id,
        hederaTransactionId: result.transactionId,
        explorerUrl: result.explorerUrl,
        simulated: result.simulated,
        status: "paid",
...
router.post("/:id/execute", requireAuth, async (req, res, next) => {
  try {
    ...
    try {
      const executionResult = await executeAgentWorkflow(
        agent,
      ...
      items: tasks.map((task) => ({
        ...
        payment: task.payment
          ? {
              ...
              hederaTransactionId: task.payment.hedera_transaction_id,
